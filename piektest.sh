...
python train.py main --config-path "$CONFIG_PATH" --data_workers=8 --rank=0.1 --index=6 --epochs=1;


CONFIG_PATH="/home/dbreen/Documents/tddl/papers/iclr_2023/configs/rn18/cifar10/decompose/dec-cp-r0.9-63.yml";
python train.py main --config-path "$CONFIG_PATH" --data_workers=8 --rank=0.9 --index=6 --epochs=1;
...
for i in {1..2};do
    ...
    python train.py main --config-path "$CONFIG_PATH" --data_workers=8 --rank=0.9 --index=$i --epochs=1;
done;
